...
$$
 \mu_{0} + 2 \times\tau_{0} = 12\] \[\mu_{0} -2\times\tau_{0} = 10\]\[2\times\mu_{0} = 12+10\]\[\mu_0 =11\] \[ \left( \mu_{0} + 2\tau_{0} \right) - \left( \mu_{0} - 2\tau_{0}\right)=12-10\] \[ 4\tau_{0}=2 \] \[ \tau_{0} = 0.5
$$
...
sigma_grid_lwr <- 1
sigma_grid_upr <- qnorm(0.99,mean =hw06_info$mu_0 ,hw06_info$tau_0)
```

Define the grid of parameter combinations.  

```{r, solution_01e_b, eval=TRUE}
param_grid <- expand.grid(mu = seq(hw06_info$mu_0 -3*hw06_info$tau_0,
                                   hw06_info$mu_0 +3*hw06_info$tau_0,
                                   length.out = 251),
                          sigma =seq(1,
                                     12.16317,
                                     length.out = 251), 
...
param_grid %>% 
  ...
  geom_point(data = tibble::tibble(xbar =10.21831, xsd =  3.204731),
...
From the figure we can see that the the sample average and sample standard deviation are not the same with the mode of the posterior, they deviated from the posterior mode.  
...
From homework5 question 2b we can get the following first derivative,
$$
 \frac{d}{d\mu} \left( \log \left( p\left(\mu \mid \mathbf{x}, \sigma\right) \right)\right) = \frac{N}{ \sigma^{2}} \left( \bar{x} - \mu \right) - \frac{1}{ \tau_{0}^{2}} \left(\mu - \mu_{0} \right)\] then the second derivative will be, \[\frac{d^2}{d\mu^2}\left( \log \left(p\left(\mu \mid \mathbf{x},\sigma\right)\right)\right)=\frac{d}{d\mu}\left(\frac{N}{ \sigma^{2}} \left( \bar{x} - \mu \right) - \frac{1}{ \tau_{0}^{2}} \left(\mu - \mu_{0} \right)\right)\] \[\frac{d^2}{d\mu^2}\left( \log \left(p\left(\mu \mid \mathbf{x},\sigma\right)\right)\right)=\frac{N}{ \sigma^{2}}\frac{d}{d\mu}\left(-\mu \right) - \frac{1}{\tau_{0}^{2}}\frac{d}{d\mu}\left(\mu \right)\] we get the second derivative as, 
\[\frac{d^2}{d\mu^2}\left( \log \left(p\left(\mu \mid \mathbf{x},\sigma\right)\right)\right)=-\frac{N}{ \sigma^{2}}-\frac{1}{\tau_{0}^{2}}
$$
...
The log-posterior density function is the parabola shape with the downside direction, so the curve only has the maximum log-posterior density. Thus the mode does correspond to the $\mu$ value associated with the maximum log-posterior density.
...
Include as many equation blocks as you feel are necessary. 
$$
 p\left(\mu \mid \mathbf{x}, \sigma \right) \approx \frac{1}{\mathrm{s}_N \sqrt 2\pi }exp\left(-\frac{1}{2}\left(\frac{\mu-\mathrm{m}_N}{\mathrm{s}_N}\right)^2\right)\]\[\log \left( p\left(\mu \mid \mathbf{x}, \sigma\right) \right)\approx\log(\frac{1}{\mathrm{s}_N \sqrt 2\pi }) -\frac{1}{2}\left(\frac{\mu-\mathrm{m}_N}{\mathrm{s}_N}\right)^2\] \[\frac{d}{d\mu} \left( \log \left( p\left(\mu \mid \mathbf{x}, \sigma\right) \right)\right)\approx -\frac{1}{2{\mathrm{s}_N}^2}\times2\times(\mu-\mathrm{m}_N)\] second derivative, \[\frac{d^2}{d\mu^2}\left( \log \left(p\left(\mu \mid \mathbf{x},\sigma\right)\right)\right)\approx -\frac{1}{\mathrm{s}_N^2}\] According the conclusion from 2a, \[\frac{1}{\mathrm{s}_N^2}=\frac{N}{ \sigma^{2}}+\frac{1}{\tau_{0}^{2}}
$$
...
What do you think?  
The measurement process is essentially the same no matter what we use the Laplace approximation or not. So the likelihood distribution is not impacted by Laplace approximation. For one parameter prior distribution, Bayesian formulation is the only related to $p(\mu|\mu_0,tau_0)$, because the covariance in D x D matrix is equal to variance. So the Laplace approximation is the exact expression for the posterior mean and posterior precision.
...
What do you think?  
Because the transformation like natural log can map a lower and upper bounded variable$\sigma$ to an unbounded variable $\varphi$. 
...
$$
p \left( \mu, \varphi \mid \mathbf{x} \right) \propto \prod_{n=1}^{N} \left( \mathrm{normal} \left(x_n \mid \mu, g^{-1}(\varphi) \right) \right) \times \mathrm{normal}\left(\mu \mid \mu_0, \tau_0\right) \times \mathrm{Exp}\left(g^{-1}(\varphi)\mid \lambda=0.5\right)\times\left| \frac{d}{d \varphi} g^{-1} \left( \varphi \right) \right|
$$
...
$$
\sigma = g^{-1} \left( \varphi \right)= \exp\left(\varphi\right)\] \[\frac{d\sigma}{d\varphi} = \frac{d}{d\varphi}\left(exp(\varphi)\right)\] \[ \log \left( \frac{d}{d \varphi} g^{-1} \left( \varphi \right) \right) = \varphi \]
$$
...
log_post_cv_result <- purrr::map2_dbl(param_grid$mu, param_grid$sigma,
                                   eval_logpost,
...
cv_param_grid %>% 
  ...
  geom_point(data = tibble::tibble(xbar =10.21831 , log_xsd =log( 3.204731) ),
...
From the above results we can see that in both cases,the optimal parameter values are same and  the Hessian matrices are same. The only difference is the counts of functions and gradients, which is caused by the different position of these two initial points.  
The log-posterior surface in 1a showed us that these two initial points are at different position, the process of finding the posterior mode will will be different. In another words the two start points will go in different direction and follow difference paths.  
...
cov2cor(laplace_result$var_matrix)[1, 2]
```
...
Include as many code chunks and discussion text as you feel are necessary. 
```{r}
pnorm(mean(hw06_info$xobs), mean = laplace_result$mode[1], sqrt(diag(laplace_result$var_matrix))[1])
```
...
The posterior distribution on $\sigma$ does not look like exactly Gaussian, there is a right skewed tail.
