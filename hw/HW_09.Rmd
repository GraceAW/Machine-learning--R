...
#### SOLUTION
$$
\[ y_{n} \mid\mu_{n} \sim \mathrm{Bernouli} \left( y_{n} \mid \mu_{n} \right) \] \[ \mu_{n} = \mathrm{logit}^{-1} \left( \eta_{n} \right) \] \[ \eta_{n} = \mathbf{x}_{n,:} \boldsymbol{\beta} \] \[ p \left(\boldsymbol{\beta} \right) = \prod^{D}_{d = 0} \left( \mathrm{normal} \left( \beta_{d} \mid \mu_{\beta}, \tau_{\beta} \right) \right) \]
$$
...
#### SOLUTION
```{r}
train_df %>% ggplot(mapping = aes(x,y))+
...
What do you think?
The above graphs showed that the event probability appears to change as the input 'x' increases ,  but it is not a linear relationship. When x is less than -2 we hardly see events, the event probability is 0. When x is between -2 and 0, the event probability increase lots, and the event probability is greater than the non_event probability. When x is between 0 and 1, the event probability decrease, the non_event probability is greater than the event probability. When x is greater than 1, the event probability dominate the response.  
### 1c)
...
Xmat_linear <- model.matrix(~x,train_df)

Xmat_cubic <-model.matrix(~x+I(x^2)+I(x^3),train_df)

Xmat_spline7 <- model.matrix(~splines::ns(x,7),train_df)
```
...
we don't need to unpack the unknowns, we can use it directly.
...
logistic_logpost <- function(unknowns, my_info)
{
  ...
  log_prior <- sum(dnorm(x=unknowns[1:ncol(X)],
...
The initial guess will not impact the results, it doesn't matter which initial value you start, you will get the same results. The only parameter that contains zero in the middle 95% uncertainty interval is the intercept in the cubic model. 
...
```{r, solution_01e_b}
###
tibble::tibble(mu = laplace_linear$mode, sd = sqrt(diag(laplace_linear$var_matrix))) %>%
     mutate(lwr = mu - 2 * sd, hwr = mu + 2 * sd)
...
```{r, solution_01e_c}
### 
tibble::tibble(mu = laplace_cubic$mode, sd = sqrt(diag(laplace_cubic$var_matrix))) %>%
     mutate(lwr = mu - 2 * sd, hwr = mu + 2 * sd)
```
...
According to the results, we can see the cubic model has the highest weight, so cubic model is the best model.
...
weight_cubic
weight_spline7=exp(laplace_spline7$log_evidence) / (exp(laplace_linear$log_evidence) + exp(laplace_cubic$log_evidence) + exp(laplace_spline7$log_evidence))
weight_spline7
```
...
What do you think?  
The log-Evidence identified best model is consistent with what we got from problem 1b.It is clearly shown from Problem 1b graphs that the events probability does not follow a linear model. It is more like cubic model,because there is low events at small x, most events happens in the middle parts, and then there is a decrease in the event probability and last there is a increase again. 
...
What do you think? Why do we need a new function compared to the previous assignments?
 In the previous assignments, we can directly calculate the linear model, we will calculate the $\boldsymbol{\beta}$-parameters and $\boldsymbol{\sigma}$. But in the logistic regression, we don't need to calculate the  $\boldsymbol{\sigma}$, so we need to define a new function.
...
```{r, solution_02f_a}
###
linear_rel=post_pred_summary_linear_class_b %>% count(pred_class, y) %>% mutate(relationship = c("true negative", "false negative", "false positive", "true positive"))
linear_rel
```
...
```{r, solution_02f_b}
###
cubic_rel=post_pred_summary_cubic_class_b %>% count(pred_class, y) %>% mutate(relationship = c("true negative", "false negative", "false positive", "true positive"))
cubic_rel
```
...
```{r, solution_02f_c}
###
spline7_rel=post_pred_summary_spline7_class_b %>% count(pred_class, y) %>% mutate(relationship = c("true negative", "false negative", "false positive", "true positive"))
spline7_rel
```
...
according to the results, we can see that model with 7 degree of freedom has the highest accurary on the training set.


Add as many code chunks as you feel are necessary.  
```{r}
acc_linear=(linear_rel$n[1] +linear_rel$n[4])/sum(linear_rel$n[1:4])
acc_linear
```
```{r}
acc_cubic=(cubic_rel$n[1] +cubic_rel$n[4])/sum(cubic_rel$n[1:4])
acc_cubic
```
```{r}
acc_spline7=(spline7_rel$n[1] +spline7_rel$n[4])/sum(spline7_rel$n[1:4])
acc_spline7
```
...
```{r, solution_03a}
###
train_small%>%ggplot(mapping = aes(x,y))+
              geom_point()+
              geom_vline(xintercept=-0.6)

```

Based on the figure, we can see that the class(events from non_events) can be linearly separated, the uncertainty is enormous. So the logistic regression model will struggle with this small data set. 
...
small_laplace_from_inform<-my_laplace(rep(0,ncol(info_small_inform$design_matrix)),logistic_logpost,info_small_inform )
```


```{r, solution_03c_b, eval=TRUE}
small_laplace_from_weak <- my_laplace(rep(0,ncol(info_small_weak$design_matrix)),logistic_logpost,info_small_weak)
```
...
The results showed that the informative prior cause the posterior mode to be different from the mode associated with the very weak prior. This is because the informative prior has ruled out extreme values.
...
```{r}
small_laplace_from_inform$var_matrix %>% cov2cor()
small_laplace_from_weak$var_matrix %>% cov2cor()
```
Posterior correlation results showed that the weak prior has a higher correlation than the informative prior. When there is higher correlation, the model will be difficult to explain. 
...
What do you think? 
The weak prior can cause larger uncertainty on coefficient, which makes coefficients change in the wider range for this very small data set; however, the informative prior constraints the extreme values, which makes the coefficients lower correlation.
...
```{r, solution_03f}
###
mod_small_mle<-glm(y~x, train_small, family = "binomial")
mod_small_mle
```
...
Because the algorithm can not converge for the small number of data, so the MLE value can not be trusted.
...
It looks like that x1 and x3 have a quadratic relationship. Others are difficult to see the relationship.
...
```{r, solution_04c1}
### first corrplot
corrplot::corrplot(df %>% select(-y) %>% cor())
```


```{r, solution_04c2}
### second corrplot
corrplot::corrplot(df %>% select(-y) %>% cor(),type = "upper")
```


What do you think? 

From the plots we can see only x5 has a small correlation with other inputs, all the other inputs are highly correlated with each other.
...
```{r, solution_04d}
### add as many code chunks as you feel are necessary
Xmat <- model.matrix(~x1+x2+x3+x4+x5+x6-1, data = df%>%select(-y))
lm_info<-list(
  yobs = df$y,
  design_matrix = Xmat,
  mu_beta = 0,
...
laplace_lm <- my_laplace(rep(0, ncol(Xmat)+1), lm_logpost, lm_info)
cov2cor(laplace_lm$var_matrix)[1:6,1:6] %>% corrplot::corrplot()
```

The above graphs showed that the $\beta$ parameter posterior correlation matrix is different with the input correlation matrix.  The parameter posterior has less correlation than the input correlation.
...
```{r, solution_04e}
### add your code here
Xtrips <- model.matrix(~(.)^3-1, data = df %>% select(-y))
Xtrips
```
...
laplace_lm_trips<-my_laplace(rep(0, ncol(Xtrips)+1), lm_logpost, lm_info_trip)
cov2cor(laplace_lm_trips$var_matrix)[1:41,1:41] %>% corrplot::corrplot(type="upper",method="square")
...
```{r, solution_05b}
### your code here
enet_default <- caret::train(y ~(x1+x2+x3+x4+x5+x6)^3- 1, 
                             data = df, 
                             method = 'glmnet', 
                             metric = my_metric, 
...
According to elastic net regression equation, when $\alpha$ is 1, it is a lasso regression. 
...
enet_grid<-expand.grid(alpha = seq(0.1, 0.9, length.out = 9), 
                         lambda = seq(min(enet_default$results$lambda), 
                                      max(enet_default$results$lambda), 
                                      length.out = 25))
enet_grid
```

Tuning parameter combinations tried out are 225, and total models that will be fit should be 5625.
...
enet_tune<-caret::train(y~(x1+x2+x3+x4+x5+x6)^3 - 1, 
                          data = df, 
                          method = 'glmnet', 
                          metric = my_metric, 
                          preProcess = c('center', 'scale'), 
                          trControl = my_ctrl,
                          tuneGrid = enet_grid)
plot( log = "xTrans", enet_tune)
enet_tune$bestTune
```

What do you think?  
The identified best elastic net model more similar to lasso regression, since $\alpha$is about 0.9 which is close to 1.
...
```{r, solution_05e}
### add more code chunks if you'd like
coef(enet_tune$finalModel, s = 0.1199618)
```

What do you think?  
The coefficients that are not zero are: intercpet, x1,x3,x5,x6,x1:x3, x1:x6,x3:x5,x5:x6 and x1:x2:x6. Others are zero.
...
What do you think?  
The rankings are almost consistent with the magnitude of the coefficients printed to the screen in Problem 4e.  The coefficient for x5:x6 are the greatest one, then is the x1:x6 and x3. 
