...
Based on the figure we can see a linear relationship between the response and the input.

```{r, solution_01a}
###
train_01%>%ggplot(mapping = aes(x,y))+
           geom_point()+
...
post_sd<-sqrt(diag(laplace_01$var_matrix))[1:3]
post_sd
```
...
?  
The mean trend was used to calculate the error with the response rather than the predicted response values, because the mean trend was from the fitted model and represented the model contribution. But the predicted response values included the influence of both models and noises.
...
#### SOLUTION
the length of the `error_01_post_01` vector is 21. 
?  

```{r, solution_02d}
###
length_post<-length(error_01_post_01)
length_post
```
...
mae_01_post_01 <- sum(abs(error_01_post_01))/length_post
mae_01_post_01
mae_01_post_02 <- sum(abs(error_01_post_02))/length_post
mae_01_post_02
mae_01_post_03 <- sum(abs(error_01_post_03))/length_post
mae_01_post_03
```

? 

We are averaging over the length of the vector. The three MAE values are different, because they are from different random posterior samples. 
...
The result is consistent with the MAEs calculated in problem 2d.
...
The uncertainty of the MAE is caused by the random posterior samples.
...
###
ggplot()+
  geom_point(data=train_02,aes(x,y),color="blue")+
  geom_point(data=test_02,aes(x,y),color="red")
...
there are 50 rows in the training matrices and 50 rows in the test matrices.
...
make_spline_basis_mats <- function(J, train_data, test_data)
{
  ...
  train_matrix <- model.matrix(~splines::ns(train_data$x,knots=knots_use_basis),data =train_data) 
  
  test_matrix <- model.matrix(~splines::ns(test_data$x,knots=knots_use_basis),data =test_data) 
...
? 
150 rows in the training set and 50 rows in the test set.
...
manage_spline_fit <- function(Xtrain, logpost_func, my_settings)
{
  ...
  init_varphi <-log(rexp(n=1,rate =my_settings$design_matrix )) 
...
?  
Based on these visualizations, we can see that the models with J>7 appear to overfit the training data. From the MAE diagram we can see MAE does not significantly change when the number of features increase after J>7.  Also with the increase of the feature after J>7, the value between the test sets and training sets becomes far away from each other, which means the models can not be generalize to fit the test data. 
...
?  
Based on the visualization, model 7 and 9 has higher weights than other models, so they are better models, but model 7 has the highest weight, so model 7 is the best model.
...
#### SOLUTION
The assessments from the data split showed that the models with J less than or equal to seven are the best models, and the evidence-based assessment also showed that model 7(J) and model 9(J) are the best model. They are almost the same, the little difference are from the different assessment ways.
?  
