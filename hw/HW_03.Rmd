...
```{r}
df%>%ggplot(mapping = aes(y))+geom_bar(mapping = aes(y=stat(prop), group =1)) +geom_text(stat='count', mapping = aes(y=after_stat(count/sum(count)), label =after_stat(signif(count/sum(count), 3))), color ='blue', nudge_y =0.0275,size =5.5) +theme_bw()
```

From the graph we can see that the ratio of event and non_event is almost 1:1, so it is balanced.
...
Which model is the best?  It seems Mod_3 is the best. It's hard to be confident since the confident intervals are overlapped with each other.
...
From the Confusion Matice we can see that the False negative and False positive rates are relatively low in the  top two models with high accuracy and as the accurary of the models goes down, the False positive and negatives rates increase.
...
Which model is the best? mod_3 is the best model.  
...
mod_3 has the highest True_Positive Rate, mod_3 also has the lowest False-Positive Rate.
...
What do you think?  When the threshhold is low, most of the responses are events, that's why the sensitivity is so high.
...
What do you think?  
Model 3 is the best model. The result is consistent with the ROC AUC summary metrics calculated previously. Model 8 is closest to a "completely ineffective model", becasue it is too close to the 45 degree line and the AUC is too small.
...
What do you think?  
the sum of the probalities of event and non_event is 1 for each observation(row )
...
```{r, solution_03e}
### your code here
test_df_1_b%>%ggplot(mapping = aes(x=pred_bin)) + geom_bar(stat = "count")
```
...
The bin lies between 0.9-1 consist of only even, but no bins consists of only non_event.
...
The empirical proportions of the event is not consistent with the predicted probability for model 1
...
```{r}
my_calcurve_1<-test_df_1_b%>%count(pred_bin,y)%>%filter(y=="event")%>%mutate(prop_event=n/(test_df_1_b%>%count(pred_bin))$n)%>%mutate(mid_bin =seq(0.05,0.95, by =0.1))%>%select(pred_bin,prop_event,mid_bin)%>%tibble::tibble()
...
The calibration curve consistent with the stacked filled barchart created previously. You can find every dot on the barchart diagram.
...
```{r, solution_04e}
### your code here
test_df_3_b %>% ggplot(mapping = aes(x=pred_bin)) + geom_bar(stat="count")
```
...
```{r, solution_04f}
### your code here
test_df_8_b %>% ggplot(mapping = aes(x=pred_bin)) + geom_bar(stat="count")
```
...
#### SOLUTION
```{r}
my_calcurve_3 <- test_df_3_b %>%count(pred_bin,y)%>%filter(y=="event")%>%mutate(prop_event=n/(test_df_3_b%>%count(pred_bin))$n)%>%mutate(mid_bin =seq(0.05,0.95, by =0.1))%>%select(pred_bin,prop_event,mid_bin)%>%tibble::tibble()
```
...
```{r}
my_calcurve_8 <- test_df_8_b %>%count(pred_bin,y)%>%filter(y=="event")%>%mutate(prop_event=n/(test_df_8_b%>%count(pred_bin))$n[1:4])%>%mutate(mid_bin =seq(0.35,0.65, by =0.1))%>%select(pred_bin,prop_event,mid_bin)%>%tibble::tibble()
```
...
What do you think?
Based on the calibration curves, mode_3 is the model appears the most well calibrated since most of the points closely lies around the 45 degree line. It is relatively obvious. The most obvious aspects of performance is whether the prediction probability follows the empirical event frequency.
...
The created calibration curve is consistent with the manually created curve from the previous problems.
...
The created calibration curve is not consistent with the manually created curve from the previous problems.
