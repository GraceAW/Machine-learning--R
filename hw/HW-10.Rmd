...
```{r, solution_01a}
###
prob_01_df %>% ggplot(mapping = aes(x,f))+
               geom_point()+
               geom_line()
...
What do you think? 
The `$beta_matrix` contain 2 rows, because there are two parameters, one is the intercept and other is the slope. The `$alpha_vector` consist of 3 elements: one intercept and two coefficients for two hidden units.The first column of the $\beta$ parameter matrix is the parameters for the first hidden unit, and the second column is the parameters for the second hidden unit.
...
$$
 \mathbf{A} = \mathbf{X} \mathbf{B} \] \[ \mathbf{H} = \mathrm{logit^{-1}} \left( \mathbf{A} \right) 
$$
...
The trends of the hidden linear predictors follows the $\beta$ parameter values displayed in Problem 1b).
...
The linear portion between -1 to 1 are approximated well.
...
What do you think?  
The trends of the hidden unit values (linear and non-linear) are not consistent with the trends from Problem 1. This change was caused by the change of the slope of the hidden 1,  it changed from positive to negative, so the direction was changed. The resulting neural network may be still same with that in Problem 1, since the final results are decided by both $\beta$ and $\alpha$.
...
The above figure shows that the response trends for the two neural network are almost same.
...
What do you think?  
No, you can not "explain" or interpret the neural network behavior by ONLY examining the slopes (weights) acting on the inputs. Since the neural network behavior was determined by all the pararmeters of the two linear combinations:$\beta$ and $\alpha$ 
...
The trends of the non-linear values for the fifth hidden unit, `h_05`, relative to its "linear predictor" values, they are curved line. The differences between the two models (sets of parameters are, for param_02_5a, the slopes are most positive, so direction of the lines  goes up, and in param_02_5b, the slopes are most negative, so the lines goes to the opposite direction of that of param_02_5a
...
The two models' trends are very similar to each other. The param_02_5a model match the true sine wave better than param_02_5b. 
...
The above figure shows that the 5 hidden units performs better than the 2 hidden units. The number of hidden units controls complexity within a neural network model and  the complexity can be tuned by cross validation.
...
The slope acting on `sin(x)` is 1 which shows that  the linear model fits the sin trend very well.
...
What do you think? 
There were two unknown parameters in the linear model with the sine wave basis function. And there were 16 unknown parameters existed in the neural network with 5 hidden units, each hidden unit has 2 $\beta$ parameters, totally it has (2*5 =10)$\beta$ parameters and (5+1) $\alpha$. parameters.
...
What do you think? 
Because this is an toy model, so we know the trend of the real model. But in most of situation, we don't know what the truth is, so we need to use the  neural network to do that. 
...
```{r, solution_03a}
###
prob_03_lf%>% ggplot(mapping = aes(x=value,y))+
              geom_point()+
              facet_wrap(~name)
```
...
```
 ...
 The shape of hyperbolic tangent function is similar to the shape of the logistic function, both are s curves. The difference the two functions is the hyperolic tangent is an s-curve between -1 and 1, but the logistic is an s curve between 0 and 1. The hyperbolic rangent allows negative hidden unit responses, and logistic forces positive values. 
...
Add as many equation blocks as you feel are necessary. 
$$
 SSE = \sum_{n=1}^{N} \left( \left( y_{n} - f_{n} \right)^{2} \right) \] or \[ SSE = \left( \mathbf{y} - \mathbf{f} \right)^{T} \left( \mathbf{y} - \mathbf{f} \right) 
$$
...
### design matrix
Xmat_03 <-model.matrix(~x1+x2+x3, prob_03_df) 
...
```
What do you think?  
 Because in the neutral network, there is more than one mode, so the initial guess will impact the optimization results and might not find the global maximum. Each guess will find its local maximum.
...
The optimized parameter estimates are not same.
...
optim_fit_3_d$value
```
The results showed that the SSE of the last three initial guess are the same, which are smaller than the first one and are the better ones.
...
```
Since the 24 hidden units model has the lowest SSE value, we can think it is the best model among the models we have optimized.
...
assess_nnet_mse <- function(theta, num_hidden, transform_func, X, y)
{
  ...
  my_neuralnet_sse(theta, my_info_list) / length(y)
}
```
...
Xtest_03<-model.matrix(~x1+x2+x3, prob_03_test_df )
```
...
nnet_default
```
The best model is the one with size =5 and decay =0.1
...
The two tuning parameters in the 'nnet' package correspond to two hyper-parameters: size and decay. Size is the number of units in hidden layer and decay is the regularization parameter to avoid over-fitting.